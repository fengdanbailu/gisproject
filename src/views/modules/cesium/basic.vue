...
const entity2 = () => {
  const billboard: Cesium.Entity = viewer.entities.add({
  ...
  const label: Cesium.Entity = viewer.entities.add({
...
const entity1 = () => {
  const point: Cesium.Entity = new Cesium.Entity({
  ...
  const point2: Cesium.Entity = viewer.entities.add({
    id: "point2",
...
const test1 = () => {
  // 精度维度高度，转化为笛卡尔坐标系
  const cartesian1: Cesium.Cartesian3 = Cesium.Cartesian3.fromDegrees(
    117,
    38,
    20
  );
  const cartesian2: Cesium.Cartesian3 = Cesium.Cartesian3.fromDegrees(
    117,
    38,
    120
  );
...
const convertValue = (init: Cesium.Cartesian3) => {
  const value: Cesium.Cartographic = Cesium.Cartographic.fromCartesian(init);
  return {
    ...value,
